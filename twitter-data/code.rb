...
#YOUR CODE GOES HERE
#print each username and description
puts "Username and Description"
twitter_data.each do |hash|
  puts "#{hash.keys.first}: #{hash.values[0]["description"]}"
end
puts

#print each username and total number of followers
puts "Username and Number of Followers"
twitter_data.each do |hash|
  puts "#{hash.keys.first}: #{hash.values[0]["number of followers"]}"
end
puts

#print each username and length of last tweet
twitter_data.each do |hash|
  puts "#{hash.keys.first} last tweet was #{hash.values[0]["latest tweet"].length} characters long"
end
puts

# #print each username and total character count of all tweets
puts "Username and Total Character Count of All Tweets"
twitter_data.each do |hash|
 puts "#{hash.keys.first}: #{hash.values[0]["last 20 tweets"].to_a.join.length}"
end
puts

#which user has most followers
twitter_data.each do |hash|
  followers ["#{hash.keys.first}"] = hash.values[0]["number of followers"]
end
follower = followers.sort_by {|k,v| v}
puts "The user with the most followers is #{follower.last}"
puts

#which user has most friends
twitter_data.each do |hash|
  friends ["#{hash.keys.first}"] = hash.values[0]["number of friends"]
end
friend = friends.sort_by {|k,v| v}
puts "The user with the most friends is #{friend.last}"
puts

#which user has greatest number of tweets
twitter_data.each do |hash|
  tweets ["#{hash.keys.first}"] = hash.values[0]["number of tweets"]
end
tweet = tweets.sort_by {|k,v| v}
puts "The user with the most tweets is #{tweet.last}"
puts

#which users have a description listed in twitter_data
twitter_data.each do |hash|
  if hash.values[0]["description"] != nil
    puts "#{hash.keys.first} has a description."
  else
    puts "#{hash.keys.first} does not have a description."
  end
end
puts
#which users have a location listed in twitter_data
twitter_data.each do |hash|
  if hash.values[0]["location"] != nil
    puts "#{hash.keys.first} has a location."
  else
    puts "#{hash.keys.first} does not have a location."
  end
end
